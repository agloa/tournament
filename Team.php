...
class Team extends CRM_Contact_BAO_Group {
  ...
  /**
   * static instance to hold the C field
   *
   * @var string
   */
  static $_C = 'current_events_74'; //@todo
...
// 		}
	...
	public static function whereClause(&$params, $sortBy = TRUE, $excludeHidden = TRUE) {
		...
		$groupStatus = 1;// Active groups only; CRM_Utils_Array::value('status', $params);
		if ($groupStatus) {
			switch ($groupStatus) {
				case 1:
...
// 		}
   ...
   *   teams where fieldname = competition
	...
	public static function retrieveByCompetition($fieldName, $competition, $teamID = null){
		...
		. " JOIN civicrm_group ON civicrm_group.id = entity_id"
		. " WHERE civicrm_group.is_active = true AND " . $fieldName . " LIKE %1";
	...
	*/
	public static function OtherTeams($teamID, $playerID){
		$tournamentName = "2018";// @todo Tournament::currentTournamentName();
		...
		$team->otherTeamGamePlayerMessages('C', self::$_C, $playerID, $playerName, $tournamentName, $messages);
