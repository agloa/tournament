...
class CRM_Tournament_Player_Form_Task_AddToTeam extends CRM_Contact_Form_Task {
  ...
  private function findEligiblePlayers(){		
  	$query = 	 "SELECT id	    ,key_code FROM EligibleTeamPlayerCodes WHERE Team = %1"
  		. " UNION SELECT player, key_code FROM team_player_key_codes 	 WHERE Team = %1";
  	$params = array(1 => array($this->_id, 'Integer'));
  	$players = CRM_Core_DAO::executeQuery($query, $params);
  	
  	while ($players->fetch()) {  	
  			$this->_eligiblePlayers[$players->id] = $players->key_code;
  	}
