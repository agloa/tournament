...
function get_aclRoles($aclGroups){
	if (count($aclGroups) > 0) foreach ($aclGroups as $group){			
		$entity_id = $group["id"];		
		//$apiParams = array();
		$apiParams = array("entity_id" => $entity_id, "entity_table" => "civicrm_group", "is_active" => 1);
 ...
 */
function baseURL(){
  $wpRelativeURL = '/wp-admin/admin.php?page=CiviCRM&q=';
  $civicrm_RelativeURL = $wpRelativeURL;
	$config = CRM_Core_Config::singleton();
	$userFrameworkBaseURL = rtrim($config->userFrameworkBaseURL, "/");
	//$useFrameworkRelativeBase = rtrim($config->useFrameworkRelativeBase, "/");
	//$civiURL =  $config->civiRelativeURL;
	return "{$userFrameworkBaseURL}{$civicrm_RelativeURL}";
}
