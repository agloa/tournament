...
//       }
	...
	//This is also where you submit your Preliminary Estimates. You do need to do this once per tournament, in January.
	$billingOrgsHTML = "The next step is to double-check your organization's contact information. You probably only need to do this once, ever. <p>You are a contact for these organizations:</p><ol>";
	$billingOrgs = billing_organizations_get($cid);	
	$profile = named_profile_get("Billing Organization Profile");
	
	if (isset($billingOrgs)){
	...
	."\">Use this link to start a new team</a>.</li>"
 ...
 */
function tournament_civicrm_post($op, $objectName, $objectId, &$objectRef) {
	switch ($objectName) {
		case 'Contribution':
			$file = '/tmp/contributions.log';
			$message = strtr("Performed \"@op\" at @time on contribution #@id\n", array(
...
function tournament_civicrm_entityTypes(&$entityTypes) {
	...
	$entityTypes['CRM_Team_DAO_Team']['fields_callback'][]
