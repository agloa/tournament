...
class CRM_Tournament_Player_Form_Task_AddToTeam extends CRM_Contact_Form_Task {
  ...
  private function findEligiblePlayers() {	  	
  	$query = "SELECT Player, competition_codes FROM EligibleTeamPlayerCodes WHERE Team = {$this->_id}"
  	;
  	...
  	if (!CRM_Core_Permission::check('edit all contacts')){  		
  		...
  		$query .= " AND registration_group IN {$groups} ";
  	}  	
  	
  	$players = CRM_Core_DAO::executeQuery($query);
  	
  	while ($players->fetch()) {  	
  			$this->_eligiblePlayers[$players->Player] = $players->competition_codes;
  	}
  	
  	// Display the competition codes for team_player, i.e., players already on that team
  	$query = "SELECT player, key_code FROM team_player_key_codes WHERE Team = {$this->_id}";  	
  	 
  	$players = CRM_Core_DAO::executeQuery($query);
  	 
  	while ($players->fetch()) {
  		$this->_eligiblePlayers[$players->player] = $players->key_code;
  	}
  	
  	// Retrieve team games
  	$query = "SELECT entity_id AS ID
  	, equations_21 AS E, on_sets_22 AS O, linguishtik_23 AS L, propaganda_24 AS P
  	, presidents_25 AS M, world_events_26 AS A, wff_n_proof_27 AS W, current_events_74 AS C
  	...
  	if (strlen($teamGames->C )> 0) $this->_teamGameText .= 'C';
...
/*   	// Retrieve team games
  	...
  	$gamesWhere = $this->gamesWhere($gamesWhere, $teamGames->C, "current_events_56");
  	...
  	, equations_11 AS E, on_sets_12 AS O, linguishtik_13 AS L, propaganda_14 AS P
  	, presidents_15 AS M, world_events_16 AS A, current_events_56 AS C
  	, wff_n_proof_17 AS W
  	...
  	while ($playerRecords->fetch()) {
  		...
  		if (!isset($errors))
  		{
  			...
  			if (strlen($playerRecords->C )> 0) $gameText .= 'C';
  ...
  private static function checkTeamPlayer($teamID, $playerID, &$errors){	
